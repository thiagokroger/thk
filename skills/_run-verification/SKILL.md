...
`policies.json` is **committed** by default — `_scaffold-session` (and `install.sh`) writes a denylist-style `.gitignore` block ignoring `.thk/sessions/`, `.thk/keys/`, and `.thk/config.json` while leaving `policies.json` and `agents/*.md` untouched, so they commit with the team while transient and per-developer state stays out of git.
