...
The plan in `<contextDir>/plan.md` is the spec. This skill turns it into a working tree diff that survives `tsc --noEmit && pnpm run build`. The Hand drives — single agent, single mental model, sequential edits.
...
`run-verification` runs `pnpm i && tsc --noEmit && pnpm run build`. Pre-existing type errors on the branch are acceptable; new ones introduced by the diff are blockers.
