...
## Rules
- **Path guard.** Before `rm -rf <sessionPath>`, verify the path contains `/.thk/sessions/` as a segment. A caller passing a path outside that tree is a bug — refuse and return `{ error: "sessionPath not inside <targetRepo>/.thk/sessions/" }`. This is the last line of defense against wiping unrelated directories.
