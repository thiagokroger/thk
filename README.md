...
**Revisit mode (`/thk revisit <TICKET>`).** After Step 7 opens a Draft PR, external reviewers — typically CodeRabbit, sometimes humans, sometimes other bots — leave feedback over the next minutes to hours. The Hand never blocks waiting; revisit is a separate entry mode. Step 7d auto-schedules `/thk revisit <TICKET>` via `CronCreate` at T+`policies.review.auto_revisit_after_minutes` (default 30; null disables) so the user doesn't have to remember. The `_revisit-pr` skill is **self-contained** — works from a fresh checkout or a different machine by rehydrating the session from the GitHub issue bundle. It pulls PR comments, triages findings (accept / defer / decline), implements accepted fixes as new commits (no force-push — Draft PRs stay append-only), re-runs verification, posts a summary comment + per-thread replies, and re-bundles the session into the GitHub issue. Idempotent across rounds — invoke again later for round N+1 against new feedback.

**End-of-run summary.** When `/thk` finishes (happy path or terminal failure), the final message is one comprehensive summary — status, plain-language "what shipped", artifact links, a council-decisions table (member, decision, time spent), a numbered timeline of major events, files changed, and what's next. The user's typical workflow is "run thk, walk away for an hour, come back" — that summary is what they read on return. No follow-up questions, no /schedule prompts (Step 7d already auto-scheduled the revisit). See `skills/thk/SKILL.md#end-of-run-summary` for the format spec.
...
| `review.meeting_decision` | `_scaffold-session` (auto-bootstrap) | `"auto"` (default — Grand Maester's `assess-meeting-need` decides), `"always"` (paranoid mode — every ticket gets a meeting), `"never"` (high-velocity mode — no meetings; rely on Counselor + ad-hoc consults). |
| `review.counselor_on_simple_path` | `_scaffold-session` (auto-bootstrap) | `true` (default — recommended) — Counselor pass runs at Step 6a even on the no-meeting path. The Counselor often runs a different model than the Hand (e.g., Codex CLI in `claude_codex`), so it provides foreign-perspective oversight that same-model review misses. Set `false` only if you explicitly don't want a Counselor pass on simple tickets (e.g., Claude-only profiles where the redundancy isn't valuable). |
| `review.counselor_on_meeting_path` | `_scaffold-session` (auto-bootstrap) | `true` (default) — Counselor closes Round 2 of every meeting (this is the standard `_convene-meeting` shape). Rarely toggled. |
| `review.auto_revisit_after_minutes` | `_scaffold-session` (auto-bootstrap) | Integer minutes (default `30`) — how long after a Draft PR opens the Hand auto-schedules `/thk revisit <TICKET>` via `CronCreate`. Set `null` to disable; the Hand prints a hint instead. |
| `revisit.review_bots` / `.meeting_escalation_threshold` | `_revisit-pr` (auto-bootstrap) | Which bots count as PR review feedback (default `["coderabbitai"]`); how many accepted findings trigger a diff-phase meeting on revisit (default `3`). |
