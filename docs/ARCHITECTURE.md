...
**Database captures are judgment-driven, not URL-driven.** Unlike Jam / Figma (discovered as links in the source ticket and fetched every time they appear), DB captures are the **Grand Maester's** call, made during his `convene-meeting` review when he recognizes that a ticket or plan hinges on a specific record's state — a named user, an order ID, a subscription, a session — and the code path alone can't answer whether it's a data issue or a logic issue. He invokes the `capture-planetscale` sub-skill via `Skill` with a narrow SELECT (targeted WHERE, minimal columns, explicit `LIMIT`, one-line `purpose`). The sub-skill enforces the read-only safety gate (single-statement SELECT, banned keywords list, auto-`LIMIT 100` if absent, `employee(s)` tables banned) and auto-redacts credential-like columns (`password*`, `*_hash`, `token*`, `secret*`, etc.). The Hand does not plan, request, or dispatch DB queries — ownership lives fully with the Grand Maester.
